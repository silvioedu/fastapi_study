...
@router.post('/', status_code=status.HTTP_202_ACCEPTED, response_model=CursoModel)
...
async def post_curso(curso_id: int, curso: CursoModel, db: AsyncSession = Depends(get_session)):
  async with db as session:
    query = select(CursoModel).filter(CursoModel.id == curso_id)
    result = await session.execute(query)
